"""Standalone entry point for running the Security agent in its own terminal."""
...
def _build_response(status: str, *, report: dict | None = None, error: str | None = None) -> dict:
    payload = {
        "type": message_types.SECURITY_REVIEW_RESPONSE,
        "status": status,
    }
    if report is not None:
        payload["security_report"] = report
...
def run_agent(poll_interval: float = 0.5) -> None:
    ...
    logger = create_logger("SEC_MAIN")
    logger.info("Security agent main loop started")
    ...
    while running:
        message = bus.receive("security", timeout=poll_interval, poll_interval=poll_interval)
        if message is None:
            continue
        ...
        if msg_type == message_types.SHUTDOWN:
            ...
            bus.send(
                ...
                payload={"type": message_types.SECURITY_REVIEW_RESPONSE, "status": "SHUTDOWN_ACK"},
                conversation_id=message.conversation_id,
                in_reply_to=message.message_id,
            )
            continue

        if msg_type != message_types.REQUEST_SECURITY_REVIEW:
    ...
    logger.info("Security agent main loop stopped")


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the Security agent in standalone mode")
    ...
    try:
        ...
        print("\nSecurity agent interrupted", file=sys.stderr)
    return 0
